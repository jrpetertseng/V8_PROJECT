...
//////////////////////////////
  ...
  AUDIO_IN_EP, //AUDIO_OUT_EP,                         /* bEndpointAddress 1 out endpoint */
...
//  void *_pinpong_ptr;
  ...
  #if (0x01 == AUDIO_FS_BINTERVAL)
#pragma message "USBD_EP_TYPE_ISOC transfer"
  /* Open EP IN (mic) */
  USBD_LL_OpenEP (pdev, AUDIO_IN_EP, USBD_EP_TYPE_ISOC, AUDIO_IN_PACKET);
  //#error
  #else
#pragma message "USBD_EP_TYPE_INTR transfer"
  /* Open EP IN (mic) */
  USBD_LL_OpenEP (pdev, AUDIO_IN_EP, USBD_EP_TYPE_INTR, AUDIO_IN_PACKET);
  #error
  #endif
...
//          (uint8_t*)&haudio->in_buffer[AUDIO_IN_PACKET * !!haudio->in_buffer_half], AUDIO_IN_PACKET);
  USBD_LL_Transmit (pdev, AUDIO_IN_EP,
            (uint8_t*)_pinpong_ptr, AUDIO_IN_PACKET);
...
static uint8_t USBD_AUDIO_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  if (epnum == (AUDIO_IN_EP & 0x7F))
  {
    ...
    if(ADC_to_MIC(&_pinpong_ptr))
    {
        nUsbAudioInts+=1;
        USBD_LL_FlushEP  (pdev, AUDIO_IN_EP);
      #if UAC_USE_PCM
        USBD_LL_Transmit (pdev, AUDIO_IN_EP, (uint8_t*)(_pinpong_ptr), AUDIO_IN_PACKET);
    ...
    {
        nUsbAudioIntsNotReady+=1;
        USBD_LL_FlushEP  (pdev, AUDIO_IN_EP);
      #if UAC_USE_PCM
//        resample_linear(_pinpong_ptr, _tmp);
        USBD_LL_Transmit (pdev, AUDIO_IN_EP, (uint8_t*)(_pinpong_ptr), AUDIO_IN_PACKET);
      #else
        USBD_LL_Transmit (pdev, AUDIO_IN_EP, (uint8_t*)(haudio->in_buffer + prev), AUDIO_IN_PACKET/2);
      #endif
...
static void AUDIO_REQ_SetCurrent(USBD_HandleTypeDef *pdev, USBD_SetupReqTypedef *req)
{
  ...
  if (req->wLength != 0U)
  {
    ...
    (void)USBD_CtlPrepareRx(pdev, (uint8_t *)&VOL_CUR, req->wLength);
