...
#include <stdbool.h>
//ckhsu for SH2 interface pin definitions
...
#define RSTN_GPIO_PORT  IMU_NRST_GPIO_Port
#define RSTN_GPIO_PIN   IMU_NRST_Pin

#define BOOTN_GPIO_PORT IMU_BOOTN_GPIO_Port
#define BOOTN_GPIO_PIN  IMU_BOOTN_Pin

#define CSN_GPIO_PORT   IMU_NSS_GPIO_Port
#define CSN_GPIO_PIN    IMU_NSS_Pin

#define WAKEN_GPIO_PORT IMU_WAKE_GPIO_Port
#define WAKEN_GPIO_PIN  IMU_WAKE_Pin
...
osThreadId_t bno08xHalTaskHandle;
...
// ----------------------------------------------------------------------------------
static void bno08xHalTask(void *params);
...
static uint32_t bno08xHalTaskBuffer[ 1024 ];
osStaticThreadDef_t bno08xHalTaskControlBlock;
const osThreadAttr_t bno08xHalTask_attributes = {
  .name = "bno08xHalTask",
  .cb_mem = &bno08xHalTaskControlBlock,
  .cb_size = sizeof(bno08xHalTaskControlBlock),
  .stack_mem = &bno08xHalTaskBuffer[0],
  .stack_size = sizeof(bno08xHalTaskBuffer),
...
void sh2_hal_init()
{
    // ckhsu, this is initialized in main hal sequence.
  #if 0
    hspi2.Instance = SPI1;
    ...
    hspi2.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_32; // 2.6MHz
    ...
    // Create task
    bno08xHalTaskHandle = osThreadNew( bno08xHalTask, NULL, &bno08xHalTask_attributes);
    if (bno08xHalTaskHandle == NULL) {
...
static void spiReset(bool dfuMode)
{
    ...
    if (dfuMode) {
        ...
        hspi->Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_32;  // 84MHz / 64 -> 1.3MHz
        // hspi->Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_128; // 84MHz / 128 -> 0.65MHz
    }
